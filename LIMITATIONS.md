...
- **A `new` expression as a tag-in-script attribute value** — `query name="q" listener=new Foo();` ([#83](https://github.com/cfmleditor/tree-sitter-cfml/issues/83)). Not `query`-specific despite the issue's examples: `cflog name="q" listener=new Foo();` fails identically, so it is the generic `tag_statement` argument path rather than `query_tag`. The partial tree is what to know — `new Foo` parses as a `new_expression` and its `()` becomes `(ERROR (formal_parameters))`, so an arrow function's parameter list is out-competing the constructor arguments in that position. Raising the `prec.dynamic` on `new_expression`'s arguments does **not** fix it, which is the expected result rather than a surprise: dynamic precedence chooses between two complete parses, and here the losing reading is an error path. A fix has to remove the arrow-function reading in this position instead.
