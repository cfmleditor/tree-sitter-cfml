...
- **The `${ … }` ordered-struct literal — fixed** ([#80](https://github.com/cfmleditor/tree-sitter-cfml/issues/80)). `animals = ${ Aardwolf: "…", aardvark: "…" };` yields an `ordered_struct`, the same node the empty `[:]` and `[=]` forms already produced. **+46 parse states**, no new conflicts, zero changed trees.
  ...
  **The design question this entry used to pose is answered:** `$` stays an ordinary identifier, and only the brace form is the literal. That keeps `$ = 1`, `x = $`, `$.foo`, `$( "sel" )` and `$[ 1 ]` exactly as they were — the jQuery-shaped uses are all over the corpus — and it costs nothing to leave `$[ … ]` alone, since it already parses as a subscript.
