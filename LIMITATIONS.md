...
- **Function-listener callback on a `new` expression** — `var t = new Query():function( result, error ) { … };` (Lucee `FunctionListener.cfc`, [#87](https://github.com/cfmleditor/tree-sitter-cfml/issues/87)). The listener itself is now **supported** — see the CHANGELOG — for a target that is a call, which covers `mySuccess():…`, `arrayLen([1,2,3]):…`, `a.b.c.d():…` and `Query::new([…]):…`, and a listener that is a function literal, a struct, a component instance or a variable holding one. Only a `new` *expression* as the target is left, and it is left on cost: a bare `new` is itself a complete `new_expression`, so admitting `new_expression` into the target makes the word `New` before a `:` ambiguous with a label and with a property name wherever those are live. That needs three further conflicts and costs **578 more parse states** — 5247 → 5825, +14.9% over master, and +3.4 MB of committed C in every binding — to buy **2 nodes in one file**. Rejected on the same basis as [#75](https://github.com/cfmleditor/tree-sitter-cfml/issues/75) and the dotted struct key. **Correction:** the entry this replaces said a `function_listener` rule "fails to generate against the ternary", with the High/High rating in [`docs/FAILING-PATTERNS.md`](docs/FAILING-PATTERNS.md) "confirmed". That was wrong on both counts. The generator does offer `[$.primary_expression, $.function_listener_expression]`, it is declarable, and it is cheap: the conflict is live only on the lookahead `:` after a call, and on a synthetic workload that is 4,000 ternaries each containing a call before the colon it measured −7.2%, +1.8% and +7.7% across three runs — sign-unstable, so below the noise floor even where it is saturated. What the earlier attempt got right was that the ternary is the competing reading; what it got wrong was concluding the conflict could not be declared.
