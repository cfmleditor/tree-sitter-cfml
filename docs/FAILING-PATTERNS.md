...
| Untyped `param` shorthand ([#70](https://github.com/cfmleditor/tree-sitter-cfml/issues/70)) | 1 | 1 node, the affected file to zero. Its own `tag_statement` branch, so `param x;` yields `name:` rather than mislabelling the name a `parameter_type`. `cfscript` only |
| `default` as a declaration modifier ([#73](https://github.com/cfmleditor/tree-sitter-cfml/issues/73)) | 1 | 1 node, the affected file to zero — but two regressions on the way, both caught by the corpus scan rather than by tests: `prec.right` on the switch rules absorbed `default:` labels into the preceding case body, and `access_type` membership stopped `default` lexing as an identifier until it was named in `_reserved_identifier` |
...
| Deep run of unpaired custom tags, then a CF tag | **Med-High** | **Med** | **Cause partly known, and the previous entry over-claimed how well.** The per-tag cost is real: `1 + 1 + name + sizeof(html_depth)` in `SERIALIZE_TAGS` against tree-sitter's fixed serialization buffer, which is why the lower envelope sits at ~990. But "confirmed by the threshold tracking name length, `depth × (name + 4) ≈ 1,014` across names from 4 to 28 characters" does not survive a full sweep: across every length from 4 to 64 the product runs **938 to 4,080**, and the 28-character sample it cited is 1,184. Two of the three original samples were 4 and 10 characters, both in the range where the model does hold. The missing variable is that overflow is *survivable by design* — `_serialized` versus `_count`, with deserialize padding the difference as empty tags — so truncation costs names rather than count, and only turns fatal when a later construct needs a dropped name. Past the budget, plain text, `<div>`, an explicit close tag and EOF all parse at any depth; `<cfscript>` and `<cfoutput>` do not. So a per-tag saving (interning, hashing) moves the threshold without removing the cliff, and "rather than hunting a limit" was concluded too early — it also cannot explain a 64-character name surviving 60 levels while a 44-character one survives 20. Understand the second variable before picking an implementation. Found while fixing [#57](https://github.com/cfmleditor/tree-sitter-cfml/issues/57), the same overflow's memory-safety half. [#55](https://github.com/cfmleditor/tree-sitter-cfml/issues/55) |
...
| Untyped `param` shorthand | Low, from a review of eight new reports | **Accurate.** One `tag_statement` branch, no conflicts. The only judgement was spelling it as a branch rather than an `optional()`, so the name is not labelled a type |
| `default` as a declaration modifier | Low-Med, same review | **Under-estimated, and the review that produced the estimate is why.** The word was reachable only from `switch_default`, so admitting it to `access_type` collided with switch labels *and* stopped it lexing as an identifier. Two regressions, neither visible to `npm test` or `npm run probe` — `prec.right` broke every switch with a default clause, and four error nodes appeared in Lucee's `Administrator.cfc` and a ColdBox spec where `default` is an ordinary variable. Both were found only by the corpus scan. **A new keyword-adjacent modifier costs a `_reserved_identifier` entry and a precedence decision, and neither shows up when costing the rule in isolation**
