...
| `''` in a single-quoted tag attribute | **Low** | **Low** | The narrowest item on this page. `quoted_cf_attribute_value` already carries `'""'` in its double-quoted branch; the single-quoted branch needs the mirror alternative. Grammar only, no scanner work, no new conflict expected. [#54](https://github.com/cfmleditor/tree-sitter-cfml/issues/54) |
