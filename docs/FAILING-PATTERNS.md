...
itself wrong. Slatwall's `menu.cfm` (36 nodes) is full of
`<cf_SlatwallActionCaller …>` custom tags, and this section used to hold them up
as an obvious-looking cause that was not one: "they parse correctly in paired,
unpaired and self-closing forms". They do — **at shallow depth**, which is the
only depth that investigation tried. Unpaired custom tags nest, so a long run of
them is a deep stack, and past ~1KB of stack the file was lost entirely. Fixing
[#55](https://github.com/cfmleditor/tree-sitter-cfml/issues/55) took `menu.cfm`
from 36 nodes to **0**. The lesson is not that the reasoning was sloppy — every
form named in it really does parse — but that a construct can be cleared on
every axis someone thought to vary and still be the cause. #55 predicted this
file would fall with it, and it did.
...
| ~~Deep run of unpaired custom tags, then a CF tag~~ | ~~**Med-High**~~ **Low** | ~~**Med**~~ **Low** | **Fixed, and both the cost and the mechanism were over-rated.** The fix is ~40 lines in `common/scanner.h` with no grammar change, no new parse states and no new conflicts. The earlier entries were right that the per-tag cost is `1 + 1 + name + sizeof(html_depth)` against tree-sitter's fixed serialization buffer, and right that overflow is survivable by design — truncation costs tag *names* while `_count` keeps the depth. What neither spotted is **which** name gets dropped: the fatal tag is not one of the custom tags but the `<cfscript>` that follows them, which is itself pushed and becomes the entry that no longer fits. That is the whole of why plain text, `<div>`, an explicit close tag and EOF survive at any depth while `<cfscript>` and `<cfoutput>` do not — those four never push. It also explains why `depth × (name + 4)` fitted some name lengths and not others: the threshold is where *the next pushing tag* stops fitting, not where the run itself does. The fix keeps the stack inside what `serialize` can round-trip, with headroom reserved for the tags that legitimately nest. Corpus 722 → 682 nodes, 143 → 140 files. [#55](https://github.com/cfmleditor/tree-sitter-cfml/issues/55) |
...
| `</cfscript>` inside a string literal | **Med** | **High** | Shared-scanner work on `common/scanner.h`, so it lands in `cfml` and `cfquery` together. The raw-text scan would have to track string state, and a bug there breaks every `<cfscript>` block rather than an edge case. **Confirmed against Lucee's parser source, and the row stays.** The open question was whether the engines also end the block here, which would have made the current behaviour correct; they do not. `cfscript`'s body is `tagdependent` and delegated to `CFMLScriptTransformer`, whose `statements()` loop ends the body through `isFinish()` — `data.srcCode.isCurrent("</", data.tagName)` — called only between complete statements, never from inside expression or string-literal parsing, with no raw-text search for `</cfscript>` anywhere in the file. So Lucee has already consumed the string literal by the time it could notice the tag inside it, and this is a genuine divergence rather than an intended behaviour. RustCFML accepting the file agrees with Lucee. The reasoning is on the issue with the code quoted; note it was read from source rather than established by running Lucee, and Adobe CF was not checked. [#56](https://github.com/cfmleditor/tree-sitter-cfml/issues/56) |
...
One entry in this table is not a prediction about a fix but a claim about a
*cause*, and it failed the same way the estimates do. [#55](https://github.com/cfmleditor/tree-sitter-cfml/issues/55)'s
serialized-state overflow was written up as "confirmed by the threshold
tracking name length", on three samples. Two of them were 4 and 10 characters,
both inside the range where the model holds; the third, at 28, contradicted the
formula it was quoted as confirming, and nobody multiplied it out. Sweeping
every length from 4 to 64 took two minutes and put the product between 938 and
4,080. **A mechanism confirmed on three points is a hypothesis, and the cheap
check is to sweep the variable rather than sample it** — the same lesson the
scope matrices taught on #49, #50 and #52, arriving from the other direction.
...
- [#98](https://github.com/cfmleditor/tree-sitter-cfml/issues/98) — **no longer
  ...
  640 → 638. The lesson worth carrying: the `+591` this issue was parked on was
