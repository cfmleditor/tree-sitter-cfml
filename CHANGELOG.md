...
- Support a **statement as an arrow-function body** — `list.each( (v) => if ( v < 0 || v > 100 ) throw( message="Condition_False" ) )` ([#75](https://github.com/cfmleditor/tree-sitter-cfml/issues/75)). Two changes were needed, and the first alone was not enough, which is the part worth recording. Adding `$.if_statement` to the body `choice` made the assignment-position form parse (`x = (v) => if (v) y = 1;`) and left the reported argument-position form still failing — the body statement ends at the call's closing paren, and the scanner only offered an automatic semicolon before `}`, a newline or EOF. `scan_automatic_semicolon` now also returns at `)`. That is narrower than it sounds: the scanner is consulted only when the parser already has `AUTOMATIC_SEMICOLON` in `valid_symbols`, so an ordinary `f( a )`, a `for` header and `(1 + 2) * 3` are untouched, and all three are pinned by the corpus test. Only `if_statement` is admitted rather than a general `$.statement`, because a `statement_block` is itself a `$.statement` and the general form reopens the same ambiguity the brace-less `try` body hit. **Scanner change, so `npm run fuzz` was run as well as the usual gate.** `cfscript/src/scanner.c` only, so `cfml` and `cfquery` are untouched
- Corpus for these two: 757 → 754 error nodes across 149 → 147 files, both affected files to zero and nothing else moving
