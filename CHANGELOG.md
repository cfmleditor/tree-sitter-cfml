...
- Record the struct-literal node types as **accepted behaviour** rather than a defect ([#92](https://github.com/cfmleditor/tree-sitter-cfml/issues/92), closed as intended). `{ a = 1 }` and `{}` carry `object_pattern` / `object_assignment_pattern` / `shorthand_property_identifier_pattern`, inherited from the JavaScript fork where `=` inside braces means a default value; `{ a: 1 }` carries `object` / `pair`. Renaming them would reshape ~26k nodes and break every downstream consumer for no behavioural gain, so `LIMITATIONS.md` now states the **contract** instead: treat `object` and `object_pattern` as the same construct, and the two `shorthand_property_identifier` forms likewise, exactly as the shipped queries already do. The entry names those query lines, so a consumer can copy the pairing rather than rediscover it. No parser change
- Record that the remaining JavaScript-only rules (`with_statement`, `template_string`, `regex`, `namespace_import`, `_from_clause`) are **deliberately kept** ([#94](https://github.com/cfmleditor/tree-sitter-cfml/issues/94), closed as not planned). `LIMITATIONS.md` listed them passively as "still present and not valid CFML", which reads as an outstanding item; it now carries the decision and the reasoning. They are *unused*, not *unreachable* — each one parses today, so removing them would be a behaviour change rather than a cleanup, for a measured 43 states (0.85%). The two with any value attached are also the hardest to remove: `regex` pulls on the external `regex_pattern` token and is actively disambiguating a regex literal from division, and `namespace_import` / `_from_clause` sit inside the live `import_statement`. No parser change
- Record two constructs that **parse but produce the wrong tree** — `{ a = 1 }` and `{}` as JS destructuring patterns ([#92](https://github.com/cfmleditor/tree-sitter-cfml/issues/92), 18,234 nodes across 2,698 files and all three grammars) and a `cfml:` prefix on a static call as a JS label ([#93](https://github.com/cfmleditor/tree-sitter-cfml/issues/93), 2 nodes). Neither is a gap and neither is probe-trackable, because both parse cleanly and probes assert on error nodes. `LIMITATIONS.md` gains a section for that category, which it did not previously have — the gap lists there implied that anything absent parses correctly, and that is not true. No parser change
