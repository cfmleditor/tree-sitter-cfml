...
- Support the untyped `param` shorthand — `param x;` and `param url.number;` ([#70](https://github.com/cfmleditor/tree-sitter-cfml/issues/70)). Completes the `param` forms after [#52](https://github.com/cfmleditor/tree-sitter-cfml/issues/52): the typed shorthand and both attribute spellings already parsed. Written as its own `tag_statement` branch rather than by making the typed branch's trailing name optional, because with two bare identifiers the grammar cannot tell a type from a name and would have labelled the name a `parameter_type` — `param x;` now yields `name: (identifier)` and `param url.number;` a `member_expression`, which is what a consumer needs. Letting that branch carry attributes as well collides with the attribute-only branch on `tag name attr=v {`, so it is deliberately the bare form only. `cfscript` only, like #52: `common/define-grammar.js` has no `tag_statement` rule at all
- Support `default` as a declaration modifier — `public default any function f()` ([#73](https://github.com/cfmleditor/tree-sitter-cfml/issues/73)). Not interface-only, which the issue's framing understated: it is accepted on a component too, and with or without an access modifier in front. `$._kw_default` joins `access_type`, and the two things that made this Med rather than the Low-Med it was estimated at are both recorded in `LIMITATIONS.md`. First, `default` was previously reachable only from `switch_default`, so making it valid where a declaration can start meant `case 1: y=1; default: y=2;` could absorb the label into the case body as a modifier — `prec.right` picked exactly that reading and broke every switch with a default clause, and `prec.left` on both `switch_case` and `switch_default` is what ends the body instead. A conflict declaration cannot resolve it because the ambiguity involves a generated repeat rule. Second, the word stopped lexing as an identifier: `default = listLast( a, b )` and `default.foo` regressed in Lucee's own `Administrator.cfc` and a ColdBox spec until `default` was named in `_reserved_identifier`, the same guard `debugger` and `new` already needed. Both regressions are pinned by corpus tests rather than just fixed
- Corpus 784 → 782 error nodes across 167 → 165 files, each affected file to zero. Parse tables up 0.5% with no new conflicts declared
